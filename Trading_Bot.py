...
def connectZerodha():
    global kite
    kite=KiteConnect(api_key="q7h9r6imnv0yxv4g")
    request_token=entryToken.get()
    data=kite.generate_session(request_token,api_secret="pca98k2dk8tqzyc12elr09wspgmtp5xk")
    ...
    while(True):
        ...
        while(i < size):
            ...
            if(rsiValue <= 20):
                ...
                elif(rsiValue <= 20):  # Adjust color for RSI <= 20
                    widgets[i * 2 + 1].config(fg="blue")  # Change color for buy signal
                else:
...
stocks = ttk.Combobox(orderFrame, values=["HDFCBANK", "INFY", "TCS", "RELIANCE",  "ICICIBANK", "KOTAKBANK", "ITC", "LT", "BAJAJFINSV", "BAJFINANCE", "HINDUNILVR", "AXISBANK", "SUNPHARMA", "SBIN", "ASIANPAINT", "MARUTI", "ONGC", "BHARTIARTL", "NTPC", "TECHM", "POWERGRID", "ULTRACEMCO", "TITAN", "IOC", "NESTLEIND", "HEROMOTOCO", "GRASIM", "M&M", "DRREDDY", "INDUSINDBK", "SHREECEM", "COALINDIA", "TATASTEEL", "WIPRO", "UPL", "EICHERMOT", "CIPLA", "ADANIPORTS", "JSWSTEEL", "IOC", "BAJAJ-AUTO", "GAIL", "DIVISLAB", "ONGC", "BPCL", "HINDALCO", "HINDPETRO", "JSWSTEEL", "SBI"], width=22, font=("Arial Bold", 10))
